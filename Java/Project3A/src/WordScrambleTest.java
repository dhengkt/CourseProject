	...
	public static void wordScramble(Dictionary dic, String start, String end) {
		...
		while (!wordQ.isEmpty()) {
			...
			if (topWord.equals(end)) {
				...
				Iterator<String> itr = dic.wordsOffByOne(topWord).iterator();
				while (itr.hasNext()) {
...
class Dictionary {
	...
	private List<String> oneLetterDiffList = new LinkedList<String>();
	...
	// Getter for OneLetterDiffList.
	public String getOneLetterDiff() {
		String word = this.oneLetterDiffList.get(0);
		this.oneLetterDiffList.remove(0);

		return word;
	}

	// Setter for OneLetterDiffList.
	public void setOneLetterDiffList(String word) {

		for (String key : this.dicSet) {
			if (this.testWords(key, word)) {
				this.oneLetterDiffList.add(key);
			}
		}
	}
	...
	public void resetUsed() {
		this.dicSet.addAll(this.usedWords);
	...
	public Queue<String> wordsOffByOne(String word) {
